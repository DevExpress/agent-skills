---
name: wpf-devexpress-charts
description: Build WPF applications with the DevExpress Chart Control (ChartControl) — 2D charts including bar, line, area, pie, financial, polar, radar, funnel, box plot, point, bubble, and scatter series. Use when adding ChartControl to a WPF project; building XYDiagram2D, SimpleDiagram2D (pie/funnel), PolarDiagram2D, RadarDiagram2D; binding series to data via DataSource + ArgumentDataMember/ValueDataMember or via Diagram.SeriesItemsSource; configuring primary and secondary axes; setting axis scale types (Numerical, DateTime, TimeSpan, Qualitative); applying axis titles, custom labels, text patterns; data aggregation with AggregateFunction and ManualNumericScaleOptions/ManualDateTimeScaleOptions; adding and styling Legend; configuring tooltips and the Crosshair Cursor; enabling Single/Multiple/Extended selection. Also use when someone mentions "DevExpress WPF chart", "dxc:ChartControl", "DevExpress.Xpf.Charts", "AreaSeries2D", "LineSeries2D", "BarSideBySideSeries2D", "StockSeries2D", "PieSeries2D", "AxisX2D", "AxisY2D", "CrosshairOptions", "SeriesItemsSource". Covers both .NET (6/7/8+) and .NET Framework 4.6.2+.
