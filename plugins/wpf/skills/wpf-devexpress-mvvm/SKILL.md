---
name: wpf-devexpress-mvvm
description: Build WPF applications with the DevExpress MVVM Framework — multiple view-model strategies (compile-time source generator, runtime-generated POCO, ViewModelBase, BindableBase), DelegateCommand/AsyncCommand, predefined services (IMessageBoxService, IDialogService, IDocumentManagerService, INotificationService, INavigationService, IDispatcherService, ISplashScreenManagerService, etc.), behaviors (EventToCommand, KeyToCommand, FocusBehavior, ConfirmationBehavior, EnumItemsSourceBehavior), and view-model communication (ISupportParameter, ISupportParentViewModel, Messenger). Use when picking a view-model strategy (recommend compile-time GenerateViewModel for new projects unless another framework is already in use); wiring commands to methods via [GenerateCommand] / [Command] attributes; calling services from view models via GetService<T>(); adding behaviors via dxmvvm:Interaction.Behaviors; passing data between view models. Also use when someone mentions "ViewModelSource", "DXMessageBoxService", "DialogService", "EventToCommand", "Messenger.Default", "DevExpress.Mvvm", "dxmvvm:". Covers .NET (6/7/8+) and .NET Framework 4.6.2+.
