...
description: Expert skill for integrating DevExpress Report Viewer and Report Designer into ASP.NET Core MVC and Razor Pages applications. Use when someone needs to add a DevExpress Document Viewer or End-User Report Designer to an ASP.NET Core app; configure Program.cs with AddDevExpressControls or ConfigureReportingServices; set up npm packages, bundleconfig.json, or libman.json for reporting; implement ReportStorageWebExtension or IReportProvider; customize the viewer toolbar, tab panel, export options, or parameter editors; configure authorization, CSRF protection, or Content Security Policy for reporting; troubleshoot a blank viewer, 400/404/500 errors, "Unable to process binding", "Internal Server Error", "Report not found", CORS errors, or version mismatches; set up report caching with UseCachedReportSourceBuilder; deploy to Linux or macOS; or upgrade DevExpress reporting packages in an existing ASP.NET Core project.
compatibility: Requires .NET 8.0+. NuGet packages from the DevExpress feed (nuget.devexpress.com). Node.js (LTS) required for npm bundling. DevExpress.Drawing.Skia required for Linux/macOS hosting. All DevExpress packages must share the same version.
metadata:
  author: DevExpress
  version: "26.1"
