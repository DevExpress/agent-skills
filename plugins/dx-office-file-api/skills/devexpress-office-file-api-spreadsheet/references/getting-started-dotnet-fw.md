...
> On .NET Framework, PDF export uses Windows GDI — no additional NuGet packages are required.
    ...
    class Program
    {
        static void Main(string[] args)
        {
            using (Workbook workbook = new Workbook())
            {
                ...
                // Export to PDF (uses Windows GDI on .NET Framework)
...
| NuGet package | `DevExpress.Document.Processor` | `DevExpress.Document.Processor` |
