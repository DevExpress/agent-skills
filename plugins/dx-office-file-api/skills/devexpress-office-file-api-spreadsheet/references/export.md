...
Export workbooks as accessible PDFs using the `BeforeExport` event:
...
workbook.BeforeExport += (sender, e) =>
{
    ...
    {
        var pdfOptions = (DevExpress.XtraSpreadsheet.Export.PdfDocumentExporterOptions)e.Options;
        pdfOptions.PdfUACompatibility = PdfUACompatibility.PdfUA2; // ISO 14289-2 (v26.1+)
    }
};
workbook.ExportToPdf("accessible.pdf");
```
...
| Cross-platform rendering | Yes | Windows only |
