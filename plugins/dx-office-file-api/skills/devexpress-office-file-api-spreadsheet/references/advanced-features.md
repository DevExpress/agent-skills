...
Sort a range using `AutoFilter.SortState`. Enable AutoFilter on the range first, then apply sort conditions.
...
// Enable AutoFilter on the range, then sort column 0 (first column) descending
CellRange range = sheet["B2:E23"];
sheet.AutoFilter.Apply(range);
sheet.AutoFilter.SortState.Sort(0, true); // true = descending
```
...
// Sort descending by column 0, then descending by column 2
List<SortCondition> conditions = new List<SortCondition>
{
    new SortCondition(0, true),  // column index 0, descending
    new SortCondition(2, true)   // column index 2, descending
};
sheet.AutoFilter.SortState.Sort(conditions);
```

## Data Filtering (AutoFilter)

Enable AutoFilter on a range, then apply filter criteria to individual columns:
...
// Filter the third column (index 2) to show only values > 5000
sheet.AutoFilter.Columns[2].ApplyCustomFilter(5000, FilterComparisonOperator.GreaterThan);
```

### Filter by Multiple Values

```csharp
// Filter column 1 (index 1) to show only specific values
sheet.AutoFilter.Columns[1].ApplyFilterCriteria(
    new CellValue[] { "Widget A", "Widget B" });
```

### Clear and Disable

```csharp
// Clear active filter criteria (keeps AutoFilter arrows visible)
sheet.AutoFilter.Clear();

// Disable AutoFilter entirely (removes filter arrows)
sheet.AutoFilter.Disable();
```

## Grouping and Outlines

Group rows or columns to create collapsible outlines. Up to seven nesting levels are supported.

### Group Rows

```csharp
// Group rows 3–6 (zero-based indices 2–5), collapsed
sheet.Rows.Group(2, 5, true);  // true = collapsed

// Group rows 9–12 (indices 8–11), expanded
sheet.Rows.Group(8, 11, false);

// Create an outer group spanning rows 2–13 (indices 1–12)
sheet.Rows.Group(1, 12, false);
```

### Group Columns

```csharp
// Group columns C–F (zero-based indices 2–5), expanded
sheet.Columns.Group(2, 5, false);
```

### Ungroup Rows

```csharp
// Ungroup rows 3–6 and show previously collapsed data
sheet.Rows.UnGroup(2, 5, true);  // true = show collapsed rows

// Remove the outer row group
sheet.Rows.UnGroup(1, 12, false);
```

`UnGroup` takes the same row/column indices as `Group`. The third parameter controls whether collapsed rows are shown after ungrouping.
