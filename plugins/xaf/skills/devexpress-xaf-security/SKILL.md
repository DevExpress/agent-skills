...
description: >-
  XAF Security System — authentication, authorization, permissions, and audit trail. Covers security builder configuration (UseIntegratedMode, AddPasswordAuthentication, AddWindowsAuthentication, OAuth2 external providers via ASP.NET Core middleware), EF Core and XPO user/role setup differences, Permission Policy (DenyAllByDefault, AllowAllByDefault, ReadOnlyAllByDefault), Type/Object/Member/Navigation permissions, SecurityOperations constants, AddTypePermission, AddObjectPermission, AddObjectPermissionFromLambda, AddMemberPermission, AddMemberPermissionFromLambda, AddNavigationPermission, predefined users/roles in Updater with duplicate guards, CurrentUserId and IsCurrentUserInRole criteria functions, getting current user via SecuritySystem.CurrentUser and DI (ISecurityStrategyBase/ISecurityProvider), checking permissions with IsGrantedExtensions (CanRead/CanWrite/CanCreate/CanDelete), IsUserInRole, CreatedBy/UpdatedBy owner pattern with SetPropertyValueWithSecurityBypass for EF Core and XPO (including SecuredPropertySetter), Audit Trail module for EF Core and XPO, and security tiers (Integrated/MiddleTier/UILevel caveat).
